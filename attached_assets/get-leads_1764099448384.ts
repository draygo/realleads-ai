 ...
 * It supports filtering by status, segments, tags, and text search.
 * 
 * DEPENDENCIES:
 ...
 * - Filter by tags (e.g., 'hot', 'needs-follow-up')
 ...
 * 
 * @example
 * // Get all active leads in Pacific Heights
 * const result = await getLeadsAction(
 *   {
 *     status: 'active',
 *     segments: ['Pacific Heights']
 *   },
 *   'agent-123'
 * );
 * 
...
function buildResultMessage(leads: Lead[], params: GetLeadsParams): string {
  ...
  if (params.tags && params.tags.length > 0) {
    filterParts.push(`tags: ${params.tags.join(', ')}`);
  }
